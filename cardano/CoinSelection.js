...
const CoinSelection = {
  ...
  randomImprove: (inputs, outputs, limit, preset = []) => {
    ...
    let amount = CustomLoader.Cardano.Value.new(
      CustomLoader.Cardano.BigNum.from_str("0")
    );
    ...
    for (let i = 0; i < splitOutputsAmounts.length; i++) {
      ...
      let range = {};
      range.ideal = CustomLoader.Cardano.Value.new(
        CustomLoader.Cardano.BigNum.from_str("0")
      )
        .checked_add(splitOutputsAmounts[i])
        .checked_add(splitOutputsAmounts[i]);
      range.maximum = CustomLoader.Cardano.Value.new(
        CustomLoader.Cardano.BigNum.from_str("0")
      )
    ...
    if (utxoSelection.remaining.length > 0) {
      ...
      let minAmount = CustomLoader.Cardano.Value.new(
        CustomLoader.Cardano.min_ada_required(
          change,
      ...
      maxFee = CustomLoader.Cardano.Value.new(
        CustomLoader.Cardano.BigNum.from_str(maxFee.toString())
      );
      ...
      if (compare(change, minAmount) < 0) {
        ...
        const minAda = minAmount
          .checked_sub(CustomLoader.Cardano.Value.new(change.coin()))
          .checked_add(
            CustomLoader.Cardano.Value.new(utxoSelection.amount.coin())
          );
...
function improve(utxoSelection, outputAmount, limit, range) {
  ...
  const newAmount = CustomLoader.Cardano.Value.new(
    CustomLoader.Cardano.BigNum.from_str("0")
  )
...
function mergeOutputsAmounts(outputs) {
  let compiledAmountList = CustomLoader.Cardano.Value.new(
    CustomLoader.Cardano.BigNum.from_str("0")
  );
...
function splitAmounts(amounts) {
  ...
  if (amounts.multiasset()) {
    ...
    for (let i = 0; i < mA.keys().len(); i++) {
      ...
      for (let j = 0; j < mA.get(scriptHash).keys().len(); j++) {
        let _assets = CustomLoader.Cardano.Assets.new();
        let assetName = mA.get(scriptHash).keys().get(j);

        _assets.insert(
          CustomLoader.Cardano.AssetName.from_bytes(assetName.to_bytes()),
          CustomLoader.Cardano.BigNum.from_bytes(
            mA.get(scriptHash).get(assetName).to_bytes()
          )
        );

        let _multiasset = CustomLoader.Cardano.MultiAsset.new();
        _multiasset.insert(
          CustomLoader.Cardano.ScriptHash.from_bytes(scriptHash.to_bytes()),
          _assets
        );
        let _value = CustomLoader.Cardano.Value.new(
          CustomLoader.Cardano.BigNum.from_str("0")
        );
  ...
  splitAmounts.push(
    CustomLoader.Cardano.Value.new(
      CustomLoader.Cardano.BigNum.from_bytes(amounts.coin().to_bytes())
    )
  );
...
function isQtyFulfilled(
  ...
  if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
    let minAmount = CustomLoader.Cardano.Value.new(
      CustomLoader.Cardano.min_ada_required(
        cumulatedAmount,
        CustomLoader.Cardano.BigNum.from_str(minUTxOValue.toString())
      )
    );
...
const cloneUTxOList = (utxoList) =>
  utxoList.map((utxo) =>
    CustomLoader.Cardano.TransactionUnspentOutput.from_bytes(utxo.to_bytes())
  );
...
const cloneValue = (value) =>
  CustomLoader.Cardano.Value.from_bytes(value.to_bytes());
